...
namespace FX
{
  ...
  void CreateDepthStencilState(bool enable, bool writeEnable, ID3D11DepthStencilState* &pResult);
	...
	//we've loaded in a "blob" of compiled shader code, it needs to be set up on the gpu as a hull shader
	void CreateHullShader(char* pBuff, unsigned int buffSz, ID3D11HullShader*& pHS);
	//we've loaded in a "blob" of compiled shader code, it needs to be set up on the gpu as a domain shader
	...
	*/
	...
	class MyFX : public Singleton<MyFX>
	{
		public:
			...
			void Render(float dTime, GameModel& model, ID3D11DeviceContext *pD3DContext, MaterialExt* pOverrideMat = nullptr);
			//keep all the textures in one place then we know we only have each one once
		void RenderParticles(float dTime, ID3D11Buffer* pVBuffer, ID3D11Buffer* pIBuffer, ID3D11DeviceContext* pD3DContext, 
		...
		private:
			...
			//indicesCount is an optional parameter that facilitates multi-pass shaders
			void PreRenderObj(float dTime, ID3D11DeviceContext *pD3DContext, MaterialExt& mat);
			//mapping between vertex/index buffers and gpu
			ID3D11InputLayout* mpDefaultLayout = nullptr, *mpParticleLayout = nullptr, *mpOutlineLayout = nullptr, *mpSunLayout = nullptr;
			//a smapler to read the texture
			ID3D11SamplerState* mpSamAnisotropic = nullptr;
			//vertex, hull, domain and pixel shaders
			ID3D11VertexShader* mpVSDefault = nullptr, *mpVSParticle = nullptr, *mpVSOutline = nullptr, *mpVSSun = nullptr;
			ID3D11HullShader* mpHSSun = nullptr;
			ID3D11DomainShader* mpDSSun = nullptr;
			ID3D11PixelShader* mpPSDefault = nullptr, *mpPSParticle = nullptr, *mpPSDissolve = nullptr, *mpPSOutline = nullptr, *mpPSSun = nullptr, *mpPSLit = nullptr, *mpPSUnlit = nullptr, *mpPSLitTex = nullptr, *mpPSUnlitTex = nullptr;
			//transparency means controlling the blend states beyond default settings
			ID3D11BlendState* mpBlendTransparent = nullptr, *mpBlendAlphaTrans = nullptr, *mpBlendAdditive = nullptr;
			//multiple raster states should be available
			ID3D11RasterizerState* mpRasterStates[RasterType::MAX_STATES];
			ID3D11DepthStencilState* mpDepthStencilDisable;
		};
	SINGLETON_GET(MyFX);
