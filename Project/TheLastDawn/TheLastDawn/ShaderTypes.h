...
struct Material
{
	Material(const DirectX::SimpleMath::Vector4& a, const DirectX::SimpleMath::Vector4& d,  
		const DirectX::SimpleMath::Vector4& s, const DirectX::SimpleMath::Vector4& e)
		: Ambient(a), Diffuse(d), Specular(s), Emissive(e)
	{
		
	}
	Material() : Diffuse(1, 1, 1, 1), Ambient(1, 1, 1, 1), Specular(0, 0, 0, 1), Emissive(0, 0, 0, 1) {}
...
//Used by both the dissolve and the outline shader
struct GfxParamsPerCustomShadedObj
{
	//edgeColour for the dissolve shaders and outlineCoulour for the outline shader
	DirectX::SimpleMath::Vector3 colour = { 0.0f, 0.0f, 0.0f };
	//edgeThickness for the dissolve shaders and outlineThickness for the outline shader
	float thickness = 0.0f;
};
static_assert((sizeof(GfxParamsPerCustomShadedObj) % 16) == 0, "CB size not padded correctly");
...
struct MaterialExt
{
	...
	MaterialExt() 
		: flags(TFlags::APPEND_PATH | TFlags::LIT | TFlags::CULL | TFlags::CCW_WINDING), dissolveTimer(), passIterator(1)
	{
		blendFactors[0] = blendFactors[1] = blendFactors[2] = blendFactors[3] = 1;
		pTextureRV = nullptr;
		pNoiseMapRV = nullptr;
		pEmissionMapRV = nullptr;
	}
	MaterialExt(const Material &mat, ID3D11ShaderResourceView *pTex, TexTrsfm texTrsfm, int _flags, const std::string& _name, const std::string& file)
		: gfxData(mat), name(_name), texture(file), flags(_flags), dissolveTimer(), passIterator(1)
	{
		blendFactors[0] = blendFactors[1] = blendFactors[2] = blendFactors[3] = 1;
		pTextureRV = pTex;
		pNoiseMapRV = nullptr;
	...
	//handled by effects texture cache so don't release
	ID3D11ShaderResourceView *pTextureRV;	
	ID3D11ShaderResourceView *pNoiseMapRV;
	ID3D11ShaderResourceView *pEmissionMapRV;

	TexTrsfm texTrsfm;

	GfxParamsPerCustomShadedObj shaderVariables;

	Timer dissolveTimer;

	unsigned passIterator;
	...
	typedef enum 
	{
		...
		PARTICLE = 128,			//particles need a different vertex shader?
		ZTEST = 256,			//turn zbuffer testing on/off
		ADDITIVE = 512,			//additive blending (glow)
		DISSOLVABLE = 1024,		//has one of the dissolve pixel shaders active
		OUTLINED = 2048,		//has the outline shader on its first pass
		TESSELLATED = 4096		//goes through the tesselletation stage of the graphics pipeline
	} TFlags;
	int flags;
